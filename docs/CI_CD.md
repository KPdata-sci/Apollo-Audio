...
Runs on every push and PR to any branch, as three independent jobs:

- **test** — installs `scraper/requirements.txt` plus `pytest`/`httpx`, spins
  up a real `postgres:16-alpine` service container, applies
  `warehouse/init.sql` against it, then runs the full suite
  (`cd scraper && python -m pytest tests -q`) with `APOLLO_WAREHOUSE_DSN`
  pointed at that database. This includes
  `tests/test_warehouse_integration.py`, which normally skips itself when no
  warehouse is reachable — in CI it actually runs. No Playwright browser
  install is needed: the tests mock `app.scraping.fetch_html` rather than
